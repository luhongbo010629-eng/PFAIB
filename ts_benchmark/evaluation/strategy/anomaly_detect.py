...
class AnomalyDetect(Strategy):
    ...
    def execute(self, series_name: str, model_factory: ModelFactory) -> Any:
        ...
        try:
            ...
            detect_result = self.detect(test_data)
            if isinstance(detect_result, tuple):
                predict_labels, another = detect_result
            else:
                predict_labels = detect_result
                another = detect_result
            ...
            for ratio, predict_label in predict_labels.items():
                ...
                inference_data = [predict_label, another_to_save]
